# changed: ddr => HBM, pthread => lpthread, add save temps at line 42
# about thread: https://stackoverflow.com/questions/59053585/why-do-you-need-lpthread
...
./$(BUILDDIR)/runOnfpga_$(TARGET).xclbin: ./$(BUILDDIR)/runOnfpga_$(TARGET).xo
	v++ -l -g -t $(TARGET) -R 1 \
		...
		--report_dir ./$(BUILDDIR)/report_dir \
		--log_dir ./$(BUILDDIR)/log_dir \
		--config $(CURRENT_DIR)/connectivity_2.cfg \
		-I$(SRCDIR) \
...
./$(BUILDDIR)/runOnfpga_$(TARGET).xclbin: ./$(BUILDDIR)/runOnfpga_$(TARGET).xo
	v++ -l -g -t $(TARGET) -R 1 \
		...
		--report_dir ./$(BUILDDIR)/report_dir \
		--log_dir ./$(BUILDDIR)/log_dir \
		--config $(CURRENT_DIR)/connectivity_reduce.cfg \
		-I$(SRCDIR) \
		./$(BUILDDIR)/runOnfpga_$(TARGET).xo \
		-o ./$(BUILDDIR)/runOnfpga_$(TARGET).xclbin
