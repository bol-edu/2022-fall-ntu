...
#ifndef _blockmatmul_AB_H_
#define _blockmatmul_AB_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "blockmatmul_AB_memcore.h"

SC_MODULE(blockmatmul_AB) {
...
protected:
    ...
    // buffer instances
    blockmatmul_AB_memcore* buffer[BufferCount];
...
public:
    ~blockmatmul_AB();
    SC_CTOR(blockmatmul_AB) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "blockmatmul_AB_memcore_U_";
            name += szTmp;

            buffer[i] = new blockmatmul_AB_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "blockmatmul_AB.trace");
            // ports
