...
void blockmatmul_AB::proc_i_full_n() {
    i_full_n.write(full_n.read());
}

void blockmatmul_AB::proc_t_empty_n() {
    t_empty_n.write(empty_n.read());
}

void blockmatmul_AB::proc_i_q0() {
    i_q0.write(buf_q0[prev_iptr.read()].read());
}

void blockmatmul_AB::proc_t_q0() {
    t_q0.write(buf_q0[prev_tptr.read()].read());
}

void blockmatmul_AB::proc_i_q1() {
    i_q1.write(buf_q1[prev_iptr.read()].read());
}

void blockmatmul_AB::proc_t_q1() {
    t_q1.write(buf_q1[prev_tptr.read()].read());
}

void blockmatmul_AB::proc_buffer_signals() {
    for (unsigned i = 0; i < BufferCount; i++) {
        if (iptr.read() == i) {
...
void blockmatmul_AB::proc_count() {
    if (reset.read() == SC_LOGIC_1) {
        count.write(0);
...
void blockmatmul_AB::proc_full_n() {
    if (reset.read() == SC_LOGIC_1) {
...
void blockmatmul_AB::proc_empty_n() {
    if (reset.read() == SC_LOGIC_1) {
