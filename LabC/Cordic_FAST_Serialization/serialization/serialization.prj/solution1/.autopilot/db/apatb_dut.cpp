    ...
    string AESL_num;
{
      ...
      if (!rtl_tv_out_file.is_open()) {
        rtl_tv_out_file.open(AUTOTB_TVOUT_PC_src_buff);
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<8> > src_buff_pc_buffer(120000);
          int i = 0;
          ...
          while (AESL_token != "[[/transaction]]"){
            ...
            if (AESL_token != "") {
              src_buff_pc_buffer[i] = AESL_token.c_str();;
              i++;
            }
          ...
          if (has_unknown_value) {
            cerr << "WARNING: [SIM 212-201] RTL produces unknown value 'x' or 'X' on port " 
                 << "src_buff" << ", possible cause: There are uninitialized variables in the C design."
                 << endl; 
          }
  
          if (i > 0) {{
		    int i = 0;
            for (int j = 0, e = 120000; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_src_buff)[j*1+0] = src_buff_pc_buffer[i].range(7, 0).to_int64();
}}}
        } // end transaction
      } // end file is good
    } // end post check logic bolck
  {
      static ifstream rtl_tv_out_file;
      if (!rtl_tv_out_file.is_open()) {
        rtl_tv_out_file.open(AUTOTB_TVOUT_PC_dst_buff);
        if (rtl_tv_out_file.good()) {
          rtl_tv_out_file >> AESL_token;
          if (AESL_token != "[[[runtime]]]")
            exit(1);
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<8> > dst_buff_pc_buffer(120000);
          int i = 0;
          bool has_unknown_value = false;
          rtl_tv_out_file >> AESL_token; //data
          while (AESL_token != "[[/transaction]]"){
            ...
            // push token into output port buffer
            if (AESL_token != "") {
              dst_buff_pc_buffer[i] = AESL_token.c_str();;
              i++;
            }
          ...
          if (has_unknown_value) {
            cerr << "WARNING: [SIM 212-201] RTL produces unknown value 'x' or 'X' on port " 
                 << "dst_buff" << ", possible cause: There are uninitialized variables in the C design."
                 << endl; 
          }
  
          if (i > 0) {{
		    int i = 0;
            for (int j = 0, e = 120000; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_dst_buff)[j*1+0] = dst_buff_pc_buffer[i].range(7, 0).to_int64();
}}}
        } // end transaction
      } // end file is good
    } // end post check logic bolck
...
auto *pos = (unsigned char*)__xlx_apatb_param_src_buff+i*1;
aesl_fh.write(AUTOTB_TVIN_src_buff, formatData(pos, 8));
}
}

  tcl_file.set_num(120000, &tcl_file.src_buff_depth);
...
for (int i = 0; i < 120000; ++i) {
auto *pos = (unsigned char*)__xlx_apatb_param_dst_buff+i*1;
aesl_fh.write(AUTOTB_TVIN_dst_buff, formatData(pos, 8));
}
}

  tcl_file.set_num(120000, &tcl_file.dst_buff_depth);
...
// print src_buff Transactions
{
aesl_fh.write(AUTOTB_TVOUT_src_buff, begin_str(AESL_transaction));
if (__xlx_apatb_param_src_buff) {
for (int i = 0; i < 120000; ++i) {
auto *pos = (unsigned char*)__xlx_apatb_param_src_buff+i*1;
aesl_fh.write(AUTOTB_TVOUT_src_buff, formatData(pos, 8));
}
}

  tcl_file.set_num(120000, &tcl_file.src_buff_depth);
aesl_fh.write(AUTOTB_TVOUT_src_buff, end_str());
}

// print dst_buff Transactions
{
aesl_fh.write(AUTOTB_TVOUT_dst_buff, begin_str(AESL_transaction));
if (__xlx_apatb_param_dst_buff) {
for (int i = 0; i < 120000; ++i) {
auto *pos = (unsigned char*)__xlx_apatb_param_dst_buff+i*1;
aesl_fh.write(AUTOTB_TVOUT_dst_buff, formatData(pos, 8));
}
}

  tcl_file.set_num(120000, &tcl_file.dst_buff_depth);
aesl_fh.write(AUTOTB_TVOUT_dst_buff, end_str());
}
