...
void XDut_Set_size(XDut *InstancePtr, u32 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XDut_WriteReg(InstancePtr->Control_BaseAddress, XDUT_CONTROL_ADDR_SIZE_DATA, Data);
}

u32 XDut_Get_size(XDut *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XDut_ReadReg(InstancePtr->Control_BaseAddress, XDUT_CONTROL_ADDR_SIZE_DATA);
    return Data;
}
