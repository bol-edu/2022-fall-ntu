...
# 2 "<built-in>" 2
# 1 "HLS_Final_vitis_src/dpu.cpp" 2
...
# 2 "HLS_Final_vitis_src/dpu.cpp" 2
...
# 3 "HLS_Final_vitis_src/dpu.cpp" 2
...
# 4 "HLS_Final_vitis_src/dpu.cpp" 2
...
DPU::DPU(){
 VITIS_LOOP_40_1: for(int i=0;i<256;i++) pMem[5*6 + 3*5 + 5*6 + 1 + 6*10][i] = 8380417;
 VITIS_LOOP_41_2: for(int i=0;i<256;i++) pMem[5*6 + 3*5 + 5*6 + 1 + 6*10 + 1][i] = 58728449;
 VITIS_LOOP_42_3: for(int i=0;i<8;i++){
  VITIS_LOOP_43_4: for(int j=0;j<256;j++){
   if(j<128) pMem[5*6 + 3*5 + 5*6 + 1 + 6*10 + 2 +i][j] = -4186625;
   else pMem[5*6 + 3*5 + 5*6 + 1 + 6*10 + 2 +i][j] = zetas[(1<<i) + ((j-128) % (1<<i))];
  }
 }
 VITIS_LOOP_48_5: for(int i=7;i>=0;i--){
  VITIS_LOOP_49_6: for(int j=0;j<256;j++){
   if(j<128) pMem[5*6 + 3*5 + 5*6 + 1 + 6*10 + 10 +(7-i)][j] = -4186625;
   else pMem[5*6 + 3*5 + 5*6 + 1 + 6*10 + 10 +(7-i)][j] = -zetas[(1<<(i+1)) - 1 - ((j-128) % (1<<i))];
  }
 }
 VITIS_LOOP_54_7: for(int i=0;i<256;i++) pMem[5*6 + 3*5 + 5*6 + 1 + 6*10 + 18][i] = 41978;
 VITIS_LOOP_55_8: for(int i=0;i<256;i++) pMem[5*6 + 3*5 + 5*6 + 1 + 6*10 + 19][i] = 1025;
 VITIS_LOOP_56_9: for(int i=0;i<256;i++) pMem[5*6 + 3*5 + 5*6 + 1 + 6*10 + 20][i] = 2*((8380417 -1)/32);
}
void DPU::read_p1(uint8_t addr){
 VITIS_LOOP_59_1: for(int i=0;i<256;i++) {
...
void DPU::read_p2(uint8_t addr){
 VITIS_LOOP_66_1: for(int i=0;i<256;i++){
#pragma HLS unroll factor=256
 p2[i] = pMem[addr][i];
 }
}
...
void DPU::read_p5(uint8_t addr){
 VITIS_LOOP_73_1: for(int i=0;i<256;i++) p5[i] = pMem[addr][i];
}

void DPU::write_p3(uint8_t addr){
 VITIS_LOOP_77_1: for(int i=0;i<256;i++){
#pragma HLS unroll factor=256
 pMem[addr][i] = p3[i];
 }
}
...
void DPU::write_p4(uint8_t addr){
 VITIS_LOOP_84_1: for(int i=0;i<256;i++){
#pragma HLS unroll factor=256
 pMem[addr][i] = p4[i];
 }
}
...
void DPU::write_p5(uint8_t addr){
 VITIS_LOOP_91_1: for(int i=0;i<256;i++) pMem[addr][i] = p5[i];
}

void DPU::write_Mem(int32_t* a, uint8_t addr){
 VITIS_LOOP_95_1: for(int i=0;i<256;i++) pMem[addr][i] = a[i];
}
void DPU::read_Mem(int32_t* a, uint8_t addr){
 VITIS_LOOP_98_1: for(int i=0;i<256;i++) a[i] = pMem[addr][i];
}

void DPU::read_ntt(uint8_t addr){
 VITIS_LOOP_102_1: for(int i=0;i<256;i++){
#pragma HLS unroll factor=256
...
void DPU::read_intt(uint8_t addr){
 VITIS_LOOP_117_1: for(int i=0;i<256;i++){
#pragma HLS unroll factor=256
 int32_t tmp = pMem[addr][i];
  if(i%2==0){
   p1[(i>>1)] = tmp;
...
#pragma HLS DEPENDENCE dependent=false type=inter variable=pMem
 read_p1(5*6 + 3*5 + 5*6 + 1 +i+6);
   ...
   break;
# 528 "HLS_Final_vitis_src/dpu.cpp"
 }
}
 ...
 int64_t tmp;
  switch(type){
   ...
   case 8:
    ...
    break;
# 594 "HLS_Final_vitis_src/dpu.cpp"
  }
 }
}
...
void DPU::dpu_pack(const uint8_t addr, uint8_t rb[1024], uint8_t pb[1024], int ptrs, uint8_t type, const int itr){
 ...
 switch(type){
  case 0:
   VITIS_LOOP_603_1: for(int j=0; j<6; j++){
    read_p5(addr+j);
    VITIS_LOOP_605_2: for(int i=0; i < 256/4; ++i) {
     pb[ptr+0] = (p5[4*i+0] >> 0);
   ...
   break;
  case 1:
   VITIS_LOOP_616_3: for(int j=0; j<6; j++){
    VITIS_LOOP_617_4: for(int i = 0; i < 256/4; ++i) {
   ...
   break;
  case 2:
   VITIS_LOOP_628_5: for(int j=0; j<6; j++){
    read_p5(addr+j);
    VITIS_LOOP_630_6: for(int i = 0; i < 256/8; ++i) {
     uint32_t t[8];
     t[0] = (1 << (13 -1)) - p5[8*i+0];
     t[1] = (1 << (13 -1)) - p5[8*i+1];
     t[2] = (1 << (13 -1)) - p5[8*i+2];
   ...
   break;
  case 3:
   VITIS_LOOP_665_7: for(int j=0; j<6; j++){
    VITIS_LOOP_666_8: for(int i = 0; i < 256/8; ++i) {
     uint32_t t[8];
   ...
   break;
  case 4:
   VITIS_LOOP_691_9: for(int j=0; j<5; j++){
    read_p5(addr+j);
    VITIS_LOOP_693_10: for(int i = 0; i < 256/2; ++i) {
     uint32_t t[2];
     t[0] = (1 << 19) - p5[2*i+0];
     t[1] = (1 << 19) - p5[2*i+1];
     pb[ptr+0] = t[0];
   ...
   break;
  case 5:
   VITIS_LOOP_708_11: for(int j=0; j<5; j++){
    VITIS_LOOP_709_12: for(int i = 0; i < 256/2; ++i) {
     uint32_t t[2];
   ...
   break;
  case 6:
   VITIS_LOOP_721_13: for(int j=0; j<itr; j++){
    read_p5(addr+j);
    VITIS_LOOP_723_14: for(int i = 0; i < 256/2; ++i) {
     uint32_t t[2];
     t[0] = 4 - p5[2*i+0];
     t[1] = 4 - p5[2*i+1];
     pb[ptr] = t[0] | (t[1] << 4);
   ...
   break;
  case 7:
   VITIS_LOOP_733_15: for(int j=0; j<itr; j++){
    VITIS_LOOP_734_16: for(int i = 0; i < 256/2; ++i) {
     uint32_t t[2];
     t[0] = pb[ptr] & 0x0F;
     t[1] = pb[ptr] >> 4;
     p5[2*i+0] = 4 - t[0];
   ...
   break;
  case 8:
   VITIS_LOOP_746_17: for(int j=0; j<6; j++){
    read_p5(addr+j);
      VITIS_LOOP_748_18: for(int i = 0; i < 256/2; ++i){
       pb[ptr] = p5[2*i+0] | (p5[2*i+1] << 4);
       ptr += 1;
      }
   }
   break;
# 769 "HLS_Final_vitis_src/dpu.cpp"
  case 11:
   norm_err = 0;
   VITIS_LOOP_771_19: for(int j=0; j<itr; j++){
    read_p5(addr+j);
    VITIS_LOOP_773_20: for(int i = 0; i < 256; ++i){
  ...
  case 12:{
   unsigned int k = 0;
   VITIS_LOOP_785_21: for(int unsigned i = 0; i < 55 + 6; ++i) {pb[i] = 0;}
   VITIS_LOOP_786_22: for(int unsigned i = 0; i < 6; ++i){
    read_p5(addr+i);
    VITIS_LOOP_788_23: for(unsigned int j = 0; j < 256; ++j) if(p5[j] != 0) pb[k++] = j;
    pb[55 + i] = k;
   }
   break;}
  ...
  case 13:{
   ...
   uint8_t* pb2 = pb + ptr;
   sig_err = 0;
   VITIS_LOOP_797_24: for(int unsigned i = 0; i < 6; ++i){
    VITIS_LOOP_798_25: for(unsigned int j = 0; j < 256; ++j) p5[j] = 0;
    if(pb2[55 + i] < k || pb2[55 + i] > 55){
     sig_err = 1;
     break;
    }
    VITIS_LOOP_803_26: for(unsigned int j = k; j < pb2[55 + i]; ++j){
   ...
   if(sig_err==0){
    VITIS_LOOP_815_27: for(unsigned int j = k; j < 55; ++j){
     if(pb2[j]){
      sig_err = 1;
      break;
     }
  ...
  case 14:{
   hn = 0;
   VITIS_LOOP_826_28: for(int j=0; j<6; j++){
    read_p5(addr+j);
      VITIS_LOOP_828_29: for(int i = 0; i < 256; ++i){
       if(p5[i]==1) hn += 1;
      }
   }
