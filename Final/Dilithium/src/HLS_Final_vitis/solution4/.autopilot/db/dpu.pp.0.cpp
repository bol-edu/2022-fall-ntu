...
#pragma HLS INLINE
 VITIS_LOOP_60_1: for(int i=0;i<256;i++) {
...
#pragma HLS INLINE
 VITIS_LOOP_68_1: for(int i=0;i<256;i++){
#pragma HLS unroll factor=256
 p2[i] = pMem[addr][i];
 }
}
...
void DPU::read_p5(uint8_t addr){
 VITIS_LOOP_75_1: for(int i=0;i<256;i++) p5[i] = pMem[addr][i];
}

void DPU::write_p3(uint8_t addr){
#pragma HLS INLINE
 VITIS_LOOP_80_1: for(int i=0;i<256;i++){
#pragma HLS unroll factor=256
 pMem[addr][i] = p3[i];
 }
}
...
#pragma HLS INLINE
 VITIS_LOOP_88_1: for(int i=0;i<256;i++){
#pragma HLS unroll factor=256
 pMem[addr][i] = p4[i];
 }
}
...
void DPU::write_p5(uint8_t addr){
 VITIS_LOOP_95_1: for(int i=0;i<256;i++) pMem[addr][i] = p5[i];
}

void DPU::write_Mem(int32_t* a, uint8_t addr){
 VITIS_LOOP_99_1: for(int i=0;i<256;i++) pMem[addr][i] = a[i];
}
void DPU::read_Mem(int32_t* a, uint8_t addr){
 VITIS_LOOP_102_1: for(int i=0;i<256;i++) a[i] = pMem[addr][i];
}

void DPU::read_ntt(uint8_t addr){
#pragma HLS INLINE
 VITIS_LOOP_107_1: for(int i=0;i<256;i++){
#pragma HLS unroll factor=256
...
#pragma HLS INLINE
 VITIS_LOOP_123_1: for(int i=0;i<256;i++){
#pragma HLS unroll factor=256
 int32_t tmp = pMem[addr][i];
  if(i%2==0){
   p1[(i>>1)] = tmp;
...
void DPU::dpu_pack(const uint8_t addr, uint8_t rb[1024], uint8_t pb[1024], int ptrs, uint8_t type, const int itr){
 ...
 ptr = ptrs;
 if(type==0){
  VITIS_LOOP_431_1: for(int j=0; j<6; j++){
   read_p5(addr+j);
   VITIS_LOOP_433_2: for(int i=0; i < 256/4; ++i) {
    pb[ptr+0] = (p5[4*i+0] >> 0);
  ...
  VITIS_LOOP_445_3: for(int j=0; j<6; j++){
   VITIS_LOOP_446_4: for(int i = 0; i < 256/4; ++i) {
  ...
  VITIS_LOOP_458_5: for(int j=0; j<6; j++){
   read_p5(addr+j);
   VITIS_LOOP_460_6: for(int i = 0; i < 256/8; ++i) {
    uint32_t t[8];
    t[0] = (1 << (13 -1)) - p5[8*i+0];
    t[1] = (1 << (13 -1)) - p5[8*i+1];
    t[2] = (1 << (13 -1)) - p5[8*i+2];
  ...
  VITIS_LOOP_496_7: for(int j=0; j<6; j++){
   VITIS_LOOP_497_8: for(int i = 0; i < 256/8; ++i) {
    uint32_t t[8];
  ...
  VITIS_LOOP_523_9: for(int j=0; j<5; j++){
   read_p5(addr+j);
   VITIS_LOOP_525_10: for(int i = 0; i < 256/2; ++i) {
    uint32_t t[2];
    t[0] = (1 << 19) - p5[2*i+0];
    t[1] = (1 << 19) - p5[2*i+1];
    pb[ptr+0] = t[0];
  ...
  VITIS_LOOP_542_11: for(int j=0; j<5; j++){
   VITIS_LOOP_543_12: for(int i = 0; i < 256/2; ++i) {
    uint32_t t[2];
  ...
  VITIS_LOOP_556_13: for(int j=0; j<itr; j++){
   read_p5(addr+j);
   VITIS_LOOP_558_14: for(int i = 0; i < 256/2; ++i) {
    uint32_t t[2];
    t[0] = 4 - p5[2*i+0];
    t[1] = 4 - p5[2*i+1];
    pb[ptr] = t[0] | (t[1] << 4);
  ...
  VITIS_LOOP_569_15: for(int j=0; j<itr; j++){
   VITIS_LOOP_570_16: for(int i = 0; i < 256/2; ++i) {
    uint32_t t[2];
    t[0] = pb[ptr] & 0x0F;
    t[1] = pb[ptr] >> 4;
    p5[2*i+0] = 4 - t[0];
