...
void DPU::dpu_pack(const uint8_t addr, uint8_t rb[1024], uint8_t pb[1024], int ptrs, uint8_t type, const int itr){
 ...
 switch(type){
  case 0:
   VITIS_LOOP_427_1: for(int j=0; j<6; j++){
    read_p5(addr+j);
    VITIS_LOOP_429_2: for(int i=0; i < 256/4; ++i) {
     pb[ptr+0] = (p5[4*i+0] >> 0);
   ...
   break;
  case 1:
   VITIS_LOOP_440_3: for(int j=0; j<6; j++){
    VITIS_LOOP_441_4: for(int i = 0; i < 256/4; ++i) {
   ...
   break;
  case 2:
   VITIS_LOOP_452_5: for(int j=0; j<6; j++){
    read_p5(addr+j);
    VITIS_LOOP_454_6: for(int i = 0; i < 256/8; ++i) {
     uint32_t t[8];
     t[0] = (1 << (13 -1)) - p5[8*i+0];
     t[1] = (1 << (13 -1)) - p5[8*i+1];
     t[2] = (1 << (13 -1)) - p5[8*i+2];
   ...
   break;
  case 3:
   VITIS_LOOP_489_7: for(int j=0; j<6; j++){
    VITIS_LOOP_490_8: for(int i = 0; i < 256/8; ++i) {
     uint32_t t[8];
   ...
   break;
  case 4:
   VITIS_LOOP_515_9: for(int j=0; j<5; j++){
    read_p5(addr+j);
    VITIS_LOOP_517_10: for(int i = 0; i < 256/2; ++i) {
     uint32_t t[2];
     t[0] = (1 << 19) - p5[2*i+0];
     t[1] = (1 << 19) - p5[2*i+1];
     pb[ptr+0] = t[0];
   ...
   break;
  case 5:
   VITIS_LOOP_532_11: for(int j=0; j<5; j++){
    VITIS_LOOP_533_12: for(int i = 0; i < 256/2; ++i) {
     uint32_t t[2];
   ...
   break;
  case 6:
   VITIS_LOOP_545_13: for(int j=0; j<itr; j++){
    read_p5(addr+j);
    VITIS_LOOP_547_14: for(int i = 0; i < 256/2; ++i) {
     uint32_t t[2];
     t[0] = 4 - p5[2*i+0];
     t[1] = 4 - p5[2*i+1];
     pb[ptr] = t[0] | (t[1] << 4);
   ...
   break;
  case 7:
   VITIS_LOOP_557_15: for(int j=0; j<itr; j++){
    VITIS_LOOP_558_16: for(int i = 0; i < 256/2; ++i) {
     uint32_t t[2];
     t[0] = pb[ptr] & 0x0F;
     t[1] = pb[ptr] >> 4;
     p5[2*i+0] = 4 - t[0];
   ...
   break;
  case 8:
   VITIS_LOOP_570_17: for(int j=0; j<6; j++){
    read_p5(addr+j);
      VITIS_LOOP_572_18: for(int i = 0; i < 256/2; ++i){
       pb[ptr] = p5[2*i+0] | (p5[2*i+1] << 4);
       ptr += 1;
      }
   }
   break;
# 593 "HLS_Final_vitis_src/dpu.cpp"
  case 11:
   norm_err = 0;
   VITIS_LOOP_595_19: for(int j=0; j<itr; j++){
    read_p5(addr+j);
    VITIS_LOOP_597_20: for(int i = 0; i < 256; ++i){
  ...
  case 12:{
   unsigned int k = 0;
   VITIS_LOOP_609_21: for(int unsigned i = 0; i < 55 + 6; ++i) {pb[i] = 0;}
   VITIS_LOOP_610_22: for(int unsigned i = 0; i < 6; ++i){
    read_p5(addr+i);
    VITIS_LOOP_612_23: for(unsigned int j = 0; j < 256; ++j) if(p5[j] != 0) pb[k++] = j;
    pb[55 + i] = k;
   }
   break;}
  ...
  case 13:{
   ...
   uint8_t* pb2 = pb + ptr;
   sig_err = 0;
   VITIS_LOOP_621_24: for(int unsigned i = 0; i < 6; ++i){
    VITIS_LOOP_622_25: for(unsigned int j = 0; j < 256; ++j) p5[j] = 0;
    if(pb2[55 + i] < k || pb2[55 + i] > 55){
     sig_err = 1;
     break;
    }
    VITIS_LOOP_627_26: for(unsigned int j = k; j < pb2[55 + i]; ++j){
   ...
   if(sig_err==0){
    VITIS_LOOP_639_27: for(unsigned int j = k; j < 55; ++j){
     if(pb2[j]){
      sig_err = 1;
      break;
     }
  ...
  case 14:{
   hn = 0;
   VITIS_LOOP_650_28: for(int j=0; j<6; j++){
    read_p5(addr+j);
      VITIS_LOOP_652_29: for(int i = 0; i < 256; ++i){
       if(p5[i]==1) hn += 1;
      }
   }
