...
void SPU::sample_cp(int32_t *c, const uint8_t seed[32])
{
  unsigned int i, b, idx;
  uint64_t signs;
  uint8_t buf[136];

  shake_init();
  shake_absorb(256, seed, 32);
  shake_finalize(256);
  shake_squeezeblocks(256, buf, 1);

  signs = 0;
  VITIS_LOOP_556_1: for(i = 0; i < 8; ++i)
    signs |= (uint64_t)buf[i] << 8*i;
  idx = 8;

  VITIS_LOOP_560_2: for(i = 0; i < 256; ++i)
    c[i] = 0;
  VITIS_LOOP_562_3: for(i = 256 -49; i < 256; ++i) {
    VITIS_LOOP_563_4: do {
      if(idx >= 136) {
        shake_squeezeblocks(256, buf, 1);
        idx = 0;
