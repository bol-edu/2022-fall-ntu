...
__attribute__((sdx_kernel("dpu_keygen", 0))) int dpu_keygen(uint8_t pk[(32 + 6*320)], uint8_t sk[(2*32 + 48 + 5*128 + 6*128 + 6*416)], uint8_t seedbuf[3*32]) {
#line 46 "C:/Users/user/Desktop/HLS/HLS_final_vitis/solution5/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=dpu_keygen
# 8 "HLS_Final_vitis_src/wrapper.cpp"

#line 6 "C:/Users/user/Desktop/HLS/HLS_final_vitis/solution5/directives.tcl"
#pragma HLSDIRECTIVE TOP name=dpu_keygen
# 8 "HLS_Final_vitis_src/wrapper.cpp"
...
SAMPETA_LOOP1:for(unsigned int k = 0; k < 6; ++k){
  ...
  VITIS_LOOP_56_1: for(unsigned int k = 0; k < 32; ++k) pk[k] = rho[k];
  dpu.dpu_pack(5*6 + 3*5 + 3*6, 0, pk, 32, 0, 6);
  spu.shake(256, tr, 48, pk, (32 + 6*320));


  VITIS_LOOP_61_2: for(unsigned int k = 0; k < 32; ++k) sk[k] = rho[k];
  VITIS_LOOP_62_3: for(unsigned int k = 0; k < 32; ++k) sk[32 +k] = key[k];
  VITIS_LOOP_63_4: for(unsigned int k = 0; k < 48; ++k) sk[2*32 +k] = tr[k];
