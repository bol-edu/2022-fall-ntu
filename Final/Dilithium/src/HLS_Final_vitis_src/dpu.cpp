...
#pragma HLS unroll factor=256
		...
		else if(type==OP_RD32){
			p3[i] = (p1[i] + (1 << 22)) >> 23;
		}
		else if(type==OP_CADDQ){
			p3[i] = p1[i] + ((p1[i] >> 31) & Q);
		}
		else if(type==OP_PASS){
			p3[i] = p1[i];
		}
		else if(type==OP_POW2ROUND){
			p3[i] = (p1[i] + (1 << (D-1)) - 1) >> D;
			p4[i] = p1[i] - (p3[i] << D);
		}
		else if(type==OP_NTT){
