...
#pragma HLS PIPELINE II=1
	...
	calculate: for(int i=0; i<FC2_OUTPUT_SIZE; i++) {
		int sum = 0;
		sum: for(int j=0; j<FC2_INPUT_SIZE; j++) {
#pragma HLS PIPELINE II=1
			sum += data[j] * const_weight[i][j];
		}
		float out_float = round((sum - FC1_ZEROP * fc2_sum[i]) * FC1_SCALE * fc2_scale[i] / FC2_SCALE + FC2_ZEROP);
		int out_int = int(out_float); // float to integer
		DTYPE out = out_int < 0 ? 0 : (out_int > 127 ? 127 : out_int); // clip
		output_stream.write(out);
	}
...
#pragma HLS PIPELINE II=1
	...
	calculate: for(int i=0; i<FC3_OUTPUT_SIZE; i++) {
		int sum = 0;
		sum: for(int j=0; j<FC3_INPUT_SIZE; j++) {
#pragma HLS PIPELINE II=1
			sum += data[j] * const_weight[i][j];
		}
		float out = (sum - FC2_ZEROP * fc3_sum[i]) * FC2_SCALE * fc3_scale[i];
		output_stream.write(out);
	}
}
