...
#define CL_USE_DEPRECATED_OPENCL_1_2_APIS
    ...
    #define CL_DEVICE_PCIE_BDF 4384
#endif
#endif
...
constexpr size_t max_probes = 100;

int g_num_of_threads = 1;
int g_num_of_seconds = 30;
std::string g_list_name = "/home/petalinux/val.txt";
...
static std::vector<PointsList::Points> get_list(const std::string &filename){
  ...
  while (getline(fs, line)) {
      std::string filename_bin = std::string("/home/petalinux/velodyne/") + line + std::string(".bin");
      std::vector<float> PointCloud;
      int len = getfloatfilelen( filename_bin);
      PointCloud.resize( len );
      myreadfile(PointCloud.data(), len, filename_bin);
      ret.emplace_back(PointsList::Points{line, PointCloud});
      cnt++;
      if (cnt >= max_probes) break;
  }
  fs.close();
  return ret;
}
...
inline BenchMarkResult thread_main_for_performance(const PointsList *points_list,
  ...
  while (!g_stop && _counter < points_list->size() && _counter < max_probes) {
...
template <typename T>
inline int main_for_performance(int argc, char *argv[], T model) {
  parse_opt(argc, argv);
...
int main(int argc, char *argv[]) {
  ...
  std::unique_ptr<vitis::ai::PointPillars> model = vitis::ai::PointPillars::create(model1, model2);
  return vitis::ai::main_for_performance(argc, argv, std::move(model));
}
