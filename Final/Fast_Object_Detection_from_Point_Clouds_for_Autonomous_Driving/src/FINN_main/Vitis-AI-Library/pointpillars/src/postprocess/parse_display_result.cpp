...
static void draw_rgb_score_filter( V2F& fcbox, DISPLAY_PARAM& g_test,  cv::Mat& img , V1I&, V1F& final_scores, float score_thresh);
...
{
  float score_thresh = 0.5; //cfg_nms_score_threshold;
  ...
  if (flag & E_RGB) {
    ...
    draw_rgb_score_filter(final_box_preds_camera, g_test, rgb_map, res.ppresult.label_preds, res.ppresult.final_scores, score_thresh);
    __TOC__(draw_rgb)
  ...
  V3F bboxes;
  V1I vlab;
  std::tie(vlab, bboxes) = label2BevImage_v4( score_thresh, annoret);
...
void draw_rgb_score_filter( V2F& fcbox, DISPLAY_PARAM& g_test,  cv::Mat& img, V1I& label, V1F& final_scores, float score_thresh )
{
   for(unsigned int i=0; i<fcbox.size(); i++) {
