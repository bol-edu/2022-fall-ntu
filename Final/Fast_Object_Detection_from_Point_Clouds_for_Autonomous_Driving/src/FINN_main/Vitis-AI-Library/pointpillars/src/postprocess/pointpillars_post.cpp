...
#include "../types.hpp"
#include "../utils.hpp" // for load_vector()
...
int get_max3(BACKBONE_T_OUT_OUT* in){
  if (in[0] > in[1]) {
...
V1F PointPillarsPost::get_decode_box(int idx) {
  ...
  o[0] = (dpu_data.box_[  idx *7+0]*dpu_data.scale_box_ + dpu_data.offset_box_) * diagonal + g_anchor_->anchors[ idx  ][0];
  o[1] = (dpu_data.box_[  idx *7+1]*dpu_data.scale_box_ + dpu_data.offset_box_) * diagonal + g_anchor_->anchors[ idx  ][1];
  o[2] = (dpu_data.box_[  idx *7+2]*dpu_data.scale_box_ + dpu_data.offset_box_) * g_anchor_->anchors[  idx  ][5] + za;
  o[3] = exp(  (dpu_data.box_[ idx *7+3]*dpu_data.scale_box_ + dpu_data.offset_box_)) * g_anchor_->anchors[ idx ][3];
  o[4] = exp(  (dpu_data.box_[ idx *7+4]*dpu_data.scale_box_ + dpu_data.offset_box_)) * g_anchor_->anchors[ idx ][4];
  o[5] = exp(  (dpu_data.box_[ idx *7+5]*dpu_data.scale_box_ + dpu_data.offset_box_)) * g_anchor_->anchors[ idx ][5];
  o[6] = (dpu_data.box_[ idx *7+6]*dpu_data.scale_box_ + dpu_data.offset_box_) + g_anchor_->anchors[ idx ][6];
  o[2] = o[2] - o[5]/2;
  return o;
}
...
PointPillarsPost::PointPillarsPost(
      ...
      G_ANCHOR* g_anchor,
      ::second::protos::TrainEvalPipelineConfig* cfg  )
    : backbone_out_data_(backbone_out_data),
      batch_size_(batch_size),
      backbone_out_height_(backbone_out_height),
      backbone_out_width_(backbone_out_width),
      backbone_out_channel_(backbone_out_channel),
      ...
      anchors_mask( ENV_PARAM(ANCHORMASK_MAXSIZE) )
{
  ...
  // TODO: Replace these with loading Eigen Tensor this->scale_ and this->offset_
  // TODO: Replicate loaded tensors in N, H, W dimensions (but maybe Eigen would handle it itself at mul/add operations in get_dpu_data?)
  dpu_data.scale_box_ = backbone_out_scale;
  dpu_data.scale_cls_ = backbone_out_scale;
  dpu_data.scale_dir_ = backbone_out_scale;
  dpu_data.offset_box_ = backbone_out_offset;
  dpu_data.offset_cls_ = backbone_out_offset;
  dpu_data.offset_dir_ = backbone_out_offset;

  nms_confidence_ = (-log(1.0/cfg_->model().second().nms_score_threshold() -1.0 ));
  ...
  // Load post add and mul
  std::vector<float> add_;
  std::vector<float> mul_;
  load_vector(post_add_filename, add_);
  load_vector(post_mul_filename, mul_);
  // TODO: Make sanity check - length of vectors

  // Assign post add and mul to Tensors, make broadcast as mul and add are only per-channel values - shape (1, 1, 1, C_out)
  this->scale_ = Eigen::TensorMap<Eigen::Tensor<BACKBONE_T_OUT_OUT, 2, Eigen::RowMajor>>(&(mul_[0]), 1, this->backbone_out_channel_);
  this->offset_ = Eigen::TensorMap<Eigen::Tensor<BACKBONE_T_OUT_OUT, 2, Eigen::RowMajor>>(&(add_[0]), 1, this->backbone_out_channel_);
  Eigen::array<int, 2> bcast = {
    int(this->batch_size_) *
    this->backbone_out_height_ *
    this->backbone_out_width_,
    1
  };
  Eigen::Tensor<float, 2, Eigen::RowMajor> tmp_scale_ = this->scale_.broadcast(bcast);
  Eigen::Tensor<float, 2, Eigen::RowMajor> tmp_offset_ = this->offset_.broadcast(bcast);
  this->scale_ = tmp_scale_;
  this->offset_ = tmp_offset_;
}

void PointPillarsPost::get_dpu_data()
{
  constexpr long cls_channels = 2;
  constexpr long box_channels = 14;
  constexpr long dir_channels = 4;
  // size_t batch = this->batch_size_;
  ...
  // cls, box, dir
  // num anchors per loc = 2
  // number of classes = 1 ===> first 2 channels
  // box code has length 7 ===> next 14 channels
  // dir code has length 2 ===> last 4  channels

  // all dpu_data should be in NHWC
  Eigen::TensorMap<Eigen::Tensor<BACKBONE_T_OUT, 2, Eigen::RowMajor>> backbone_out_tensor(
    &(this->backbone_out_data_[0]),
    this->batch_size_ *
    this->backbone_out_height_ *
    this->backbone_out_width_,
    this->backbone_out_channel_
  );

  Eigen::Tensor<BACKBONE_T_OUT_OUT, 2, Eigen::RowMajor> bot_float = backbone_out_tensor.cast<BACKBONE_T_OUT_OUT>();

  bot_float *= this->scale_;
  bot_float += this->offset_;

  Eigen::array<long, 2> cls_start_idx = {
    0,
    0
  };
  Eigen::array<long, 2> cls_extent = {
    bot_float.dimension(0),
    cls_channels
  };

  Eigen::array<long, 2> box_start_idx = {
    0,
    cls_channels
  };
  Eigen::array<long, 2> box_extent = {
    bot_float.dimension(0),
    box_channels
  };

  Eigen::array<long, 2> dir_start_idx = {
    0,
    cls_channels + box_channels
  };
  Eigen::array<long, 2> dir_extent = {
    bot_float.dimension(0),
    dir_channels
  };

  this->backbone_out_cls_data_ = bot_float.slice(
    cls_start_idx,
    cls_extent
  );
  this->backbone_out_box_data_ = bot_float.slice(
    box_start_idx,
    box_extent
  );
  this->backbone_out_dir_data_ = bot_float.slice(
    dir_start_idx,
    dir_extent
  );

  dpu_data.cls_ = this->backbone_out_cls_data_.data();
  dpu_data.box_ = this->backbone_out_box_data_.data();
  dpu_data.dir_ = this->backbone_out_dir_data_.data();
}

PointPillarsResult PointPillarsPost::post_process( )
{

  __TIC__(getting_dpu_data)
  get_dpu_data();
  __TOC__(getting_dpu_data)
  ...
  for(int i=0; i<anchors_mask.size(); i++){
    ...
    if( float(dpu_data.cls_[   anchors_mask[i]*cfg_num_class +pos ] * dpu_data.scale_cls_ + dpu_data.offset_cls_) >= nms_confidence_) {
      top_scores.emplace_back( dpu_data.cls_[   anchors_mask[i]*cfg_num_class +pos  ] *dpu_data.scale_cls_ + dpu_data.offset_cls_);
