...
typedef struct{
 BACKBONE_T_OUT_OUT* box_;
 BACKBONE_T_OUT_OUT* cls_;
 BACKBONE_T_OUT_OUT* dir_;
 float scale_box_;
 float scale_cls_;
 float scale_dir_;
 float offset_box_;
...
typedef Eigen::Tensor<BACKBONE_T, 3, Eigen::RowMajor>       PointPillarsScatterOutDpuTensor;
typedef Eigen::TensorMap<PointPillarsScatterOutDpuTensor> PointPillarsScatterOutDpuTensorMap;
typedef Eigen::Tensor<PFN_T, 3, Eigen::RowMajor>       VoxelsTensor;
typedef Eigen::TensorMap<VoxelsTensor>                  VoxelsTensorMap;

class preout_dict 
{
    public:
        preout_dict( PFN_T* dpu_in, int s0, int s1, int s2) : // should be hwc :   12000 100  4
            voxelsData (dpu_in)
        {
