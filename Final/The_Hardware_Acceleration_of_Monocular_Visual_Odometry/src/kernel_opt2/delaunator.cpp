...
int fast_mod(const int i, const int c) {
    return i >= c ? i % c : i;
}

data_type circumradius(const Point& p1, const Point& p2, const Point& p3)
{
...
bool clockwise(data_type px, data_type py, data_type qx, data_type qy,
...
bool counterclockwise(const Point& p0, const Point& p1, const Point& p2)
{
    Point v0 = Point::vector(p0, p1);
    Point v1 = Point::vector(p0, p2);
    data_type det = Point::determinant(v0, v1);
...
bool counterclockwise(data_type px, data_type py, data_type qx, data_type qy,
    data_type rx, data_type ry)
{
    Point p0(px, py);
    Point p1(qx, qy);
...
data_type pseudo_angle(const data_type dx, const data_type dy) {
    const data_type p = dx / (abs(dx) + abs(dy));
    return (dy > 0.0 ? 3.0 - p : 1.0 + p) / 4.0; // [0..1)
}

void Delaunator::initilization()
{
    triangles_cnt = 0;
    halfedges_cnt = 0;
    m_edge_stack_size = 0;
