...
void removeOutliers (Matcher::p_match p_matched[POINT_L], int32_t *p_matched_cnt, Matcher::p_match p_matched_res[POINT_L], int32_t *p_matched_res_cnt){
    const unsigned I_POINT_L=POINT_L;

#pragma HLS INTERFACE m_axi offset = slave latency = 32 num_write_outstanding = 1 num_read_outstanding = \
    16 max_write_burst_length = 2 max_read_burst_length = 256 bundle = gmem0 port = p_matched depth = I_POINT_L * 5
#pragma HLS INTERFACE m_axi offset = slave latency = 32 num_write_outstanding = 1 num_read_outstanding = \
    16 max_write_burst_length = 2 max_read_burst_length = 256 bundle = gmem0 port = p_matched_res depth = I_POINT_L * 5
#pragma HLS INTERFACE m_axi offset = slave latency = 32 num_write_outstanding = 1 num_read_outstanding = \
    16 max_write_burst_length = 2 max_read_burst_length = 256 bundle = gmem0 port = p_matched_res_cnt depth = I_POINT_L * 5
#pragma HLS INTERFACE m_axi offset = slave latency = 32 num_write_outstanding = 1 num_read_outstanding = \
    16 max_write_burst_length = 2 max_read_burst_length = 256 bundle = gmem0 port = p_matched_cnt depth = I_POINT_L * 5
    
    //test interface
    // p_matched_res_cnt[0] = p_matched_cnt[0]+2;
    
    // do we have enough points for outlier removal?
    if (p_matched_cnt[0]<=3)
        return;
...
#pragma HLS ARRAY_PARTITION variable= num_support type=complete dim=2
    ...
    // initialize_loop: 
    for (int it = 0; it < p_matched_cnt[0]; it++) {
    ...
    for (int32_t i=0; i<d.triangles_cnt / 3; i++) {
        ...
        Matcher::p_match p1_match = p_matched[p1];


        int32_t p2 = d.triangles[i * 3 + 1];
        Matcher::p_match p2_match = p_matched[p2];

        int32_t p3 = d.triangles[i * 3 + 2];
        Matcher::p_match p3_match = p_matched[p3];
    ...
    // refill p_matched
    p_matched_res_cnt[0] = 0;
    // refill: 
    for (int i=0; i<p_matched_cnt[0]; i++)
        if (num_support[i][0] + num_support[i][1] + num_support[i][2] >=4)
            p_matched_res[p_matched_res_cnt[0]++] = p_matched[i];
  
  //----------------------------------------------------------------
  //                        bucket_features
  //----------------------------------------------------------------
  const int max_features  = 2;
  const float bucket_width  = 50;
  const float bucket_height = 50;

  // find max values
  float u_max = 0;
  float v_max = 0;
  for (int32_t it = 0; it < p_matched_res_cnt[0]; it++) {
    if (p_matched_res[it].u1c>u_max) u_max=p_matched_res[it].u1c;
    if (p_matched_res[it].v1c>v_max) v_max=p_matched_res[it].v1c;
  }
  ...
  for (int32_t it = 0; it < p_matched_res_cnt[0]; it++) {
    int32_t u = (int32_t)floor(p_matched_res[it].u1c/bucket_width);
    int32_t v = (int32_t)floor(p_matched_res[it].v1c/bucket_height);
    buckets[v*bucket_cols+u][buckets_cnt[v*bucket_cols+u]] = p_matched_res[it];
  ...
  for (int32_t i=0; i<bucket_cols*bucket_rows; i++) {
    ...
    for (int j = 0; j < buckets_cnt[i]; j++) {
    	p_matched_res[p_matched_res_cnt[0]++] = buckets[i][j];
      k++;
      if (k>=max_features)
        break;
    }
