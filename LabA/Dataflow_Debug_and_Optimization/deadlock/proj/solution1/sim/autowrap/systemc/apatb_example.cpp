...
#define WRAPC_STREAM_SIZE_OUT_B "../tv/stream_size/stream_size_out_B.dat"
#define WRAPC_STREAM_EGRESS_STATUS_B "../tv/stream_size/stream_egress_status_B.dat"
...
long __xlx_apatb_param_A_stream_buf_final_size;
{
      ...
      if (!rtl_tv_out_file.is_open()) {
        rtl_tv_out_file.open(WRAPC_STREAM_SIZE_IN_A);
  ...
  for (long i = 0; i < __xlx_apatb_param_A_stream_buf_final_size; ++i)((hls::stream<int>*)__xlx_apatb_param_A)->read();
long __xlx_apatb_param_B_stream_buf_final_size;
{
      static ifstream rtl_tv_out_file;
      if (!rtl_tv_out_file.is_open()) {
        rtl_tv_out_file.open(WRAPC_STREAM_SIZE_OUT_B);
        if (rtl_tv_out_file.good()) {
          rtl_tv_out_file >> AESL_token;
          if (AESL_token != "[[[runtime]]]")
            exit(1);
    ...
    } // end post check logic bolck
  {
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<32> > B_pc_buffer;
          int i = 0;
          ...
          while (AESL_token != "[[/transaction]]"){
            ...
            if (AESL_token != "") {
              B_pc_buffer.push_back(AESL_token.c_str());
              i++;
            }
...
((char*)&xlx_stream_elt)[0*4+0] = B_pc_buffer[j].range(7, 0).to_int64();
((char*)&xlx_stream_elt)[0*4+1] = B_pc_buffer[j].range(15, 8).to_int64();
((char*)&xlx_stream_elt)[0*4+2] = B_pc_buffer[j].range(23, 16).to_int64();
((char*)&xlx_stream_elt)[0*4+3] = B_pc_buffer[j].range(31, 24).to_int64();
...
aesl_fh.touch(WRAPC_STREAM_SIZE_IN_A);
aesl_fh.touch(WRAPC_STREAM_INGRESS_STATUS_A);
aesl_fh.touch(WRAPC_STREAM_SIZE_OUT_B);
aesl_fh.touch(WRAPC_STREAM_EGRESS_STATUS_B);
...
for (int i = 0; i < __xlx_apatb_param_A_stream_buf_final_size; ++i) {
unsigned char *pos = (unsigned char*)(__xlx_apatb_param_A_stream_buf.data()+i);
std::string s(formatData(pos, 32));
aesl_fh.write(AUTOTB_TVIN_A, s);
}

  tcl_file.set_num(__xlx_apatb_param_A_stream_buf_final_size, &tcl_file.A_depth);
...
{
aesl_fh.write(WRAPC_STREAM_INGRESS_STATUS_A, begin_str(AESL_transaction));
if (__xlx_apatb_param_A_stream_buf_final_size > 0) {
  long A_stream_ingress_size = __xlx_apatb_param_A_stream_buf_size;
sprintf(__xlx_sprintf_buffer.data(), "%d\n", A_stream_ingress_size);
 aesl_fh.write(WRAPC_STREAM_INGRESS_STATUS_A, __xlx_sprintf_buffer.data());
...
sprintf(__xlx_sprintf_buffer.data(), "%d\n", A_stream_ingress_size);
 aesl_fh.write(WRAPC_STREAM_INGRESS_STATUS_A, __xlx_sprintf_buffer.data());
  ...
  long A_stream_ingress_size = 0;
sprintf(__xlx_sprintf_buffer.data(), "%d\n", A_stream_ingress_size);
 aesl_fh.write(WRAPC_STREAM_INGRESS_STATUS_A, __xlx_sprintf_buffer.data());
}
aesl_fh.write(WRAPC_STREAM_INGRESS_STATUS_A, end_str());
}
{
aesl_fh.write(WRAPC_STREAM_SIZE_IN_A, begin_str(AESL_transaction));
sprintf(__xlx_sprintf_buffer.data(), "%d\n", __xlx_apatb_param_A_stream_buf_final_size);
 aesl_fh.write(WRAPC_STREAM_SIZE_IN_A, __xlx_sprintf_buffer.data());
aesl_fh.write(WRAPC_STREAM_SIZE_IN_A, end_str());
}
...
for (int i = 0; i < __xlx_apatb_param_B_stream_buf_final_size; ++i) {
unsigned char *pos = (unsigned char*)(__xlx_apatb_param_B_stream_buf.data()+__xlx_apatb_param_B_stream_buf_size+i);
std::string s(formatData(pos, 32));
aesl_fh.write(AUTOTB_TVOUT_B, s);
}

  tcl_file.set_num(__xlx_apatb_param_B_stream_buf_final_size, &tcl_file.B_depth);
aesl_fh.write(AUTOTB_TVOUT_B, end_str());
}

{
aesl_fh.write(WRAPC_STREAM_SIZE_OUT_B, begin_str(AESL_transaction));
sprintf(__xlx_sprintf_buffer.data(), "%d\n", __xlx_apatb_param_B_stream_buf_final_size);
 aesl_fh.write(WRAPC_STREAM_SIZE_OUT_B, __xlx_sprintf_buffer.data());
aesl_fh.write(WRAPC_STREAM_SIZE_OUT_B, end_str());
}
